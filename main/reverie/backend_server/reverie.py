...
class ReverieServer:
    ...
    def process_persona(self, persona_name):
        """
        Process the given persona by moving it in the maze and returning relevant information.

        Args:
            persona_name (str): The name of the persona to process.

        Returns:
            tuple: A tuple containing the following information:
                - persona_name (str): The name of the processed persona.
                - next_tile (tuple): The next tile the persona will move to.
                - pronunciation (str): The pronunciation of the persona's move.
                - description (str): A description of the persona's move.
                - chat (str): The persona's chat message.
        ...
        next_tile, pronunciation, description = persona.move(
            ...
            self.curr_time)
        return persona_name, next_tile, pronunciation, description, persona.scratch.chat

    async def process_persona_async(self, executor, persona_name):
            """
            Asynchronously processes the persona using the specified executor.

            Args:
                executor (concurrent.futures.Executor): The executor to use for processing.
                persona_name (str): The name of the persona to process.

            Returns:
                The result of processing the persona.
            """
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(executor, self.process_persona, persona_name)

    async def run_async_movements(self):
            """
            Runs the asynchronous movements for all personas concurrently.

            Returns:
                A list of results from each persona's asynchronous movement.
            """
            with concurrent.futures.ThreadPoolExecutor() as executor:
                tasks = [self.process_persona_async(
                    executor, persona_name) for persona_name in self.personas.keys()]
        ...
        while (self.is_running):
            ...
            if check_if_file_exists(curr_env_file):
                ...
                if env_retrieved:
                    ...
                    # NOTE: Leaving old code for baseline
                    # for persona_name, persona in self.personas.items():
                    ...
                    results = await self.run_async_movements()

                    for persona_name, next_tile, pronunciation, description, chat in results:
                        movements["persona"][persona_name] = {
                            "movement": next_tile,
                    ...
                    print(
                        f"Time for multiprocessing: {end_time_multiprocessing - start_time_multiprocessing} seconds")
                    ...
                    # Function from 'global_methods.py'
                    ...
                    # "w" means if file does not exist, create it
    ...
    def run_command(self, sim_command):
        ...
        if sim_command.lower() in ["f", "fin", "finish", "save and finish"]:
        ...
        elif sim_command.lower() == "stop":
            # Stops the simulation environment
            rs.is_running = False
            ...
            asyncio.create_task(rs.start_server(int_count))
            ...
            self.personas[" ".join(sim_command.split()[-2:])
                          ].s_mem.print_tree()
            ...
            self.personas[persona_name].open_convo_session("analysis")
              ...
              in sim_command.lower()):
            # Starts a stateful chat session with the agent.
